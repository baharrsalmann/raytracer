...
class Ray {
    ...
    HitRecord intersectionTriangle(Triangle const&triangle, const Scene& scene){
        ...
        if (dot(hit.normal,scene.cameras[0].gaze) > 0) return hit;
        if (beta+gama <= 1 && 0 <= beta && 0 <= gama && t>0){ 
...
int main()
{
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        {   
            Ray ray = Ray::cameraToPixel(cam,x,y);
    ...
    write_ppm("monkey2.ppm", image, width, height);}
